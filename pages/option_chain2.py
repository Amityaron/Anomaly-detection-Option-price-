...
if ticker:
    try:
        ...
        if load_button:
            ...
            # Save option-chain data so it stays visible after Streamlit reruns
        ...
        # Display saved option-chain table even after calculator clicks
...
st.write(
    ...
    "3 legs or 4 legs. DTE is taken from the loaded option-chain expiration."
)

# IMPORTANT:
# This selectbox is OUTSIDE the form,
...
with st.form("manual_put_ladder_form"):
    ...
    if loaded_option_dte is not None:
        ladder_dte = int(loaded_option_dte)
        top_col2.number_input(
            ...
            step=1,
            disabled=True,
            help="Using the same DTE as the loaded option-chain expiration."
        )
    else:
        ladder_dte = top_col2.number_input(
            ...
            value=30,
            step=1,
            help="Load an option chain first to use its DTE automatically."
        )

    if st.session_state.get("option_expiration") is not None:
        st.caption(
            f"Using loaded expiration: {st.session_state.option_expiration} | "
            f"DTE: {st.session_state.option_dte}"
        )
    else:
        st.caption("No option chain loaded yet. Calculator uses manual DTE.")
