...
st.subheader('Based on Diff Z-Score 22 Days')
...
for etf in etfs:
    ...
    # Download enough data to calculate:
    # 1. 22-day price z-score
    # 2. 22-day difference z-score
    ...
    try:
        ...
        if len(data) < 44:
            st.warning(
                f"Not enough data for {etf} to calculate 22-day indicators. Skipping."
            )
            continue
        ...
        # ================================
        # Stock Price Z-Score 22 Days
        # ================================

        last_22_prices = data.tail(22)

        mean_price_22 = last_22_prices.mean()
        std_price_22 = last_22_prices.std()

        stock_price_z_score_22 = (
            round((current_price - mean_price_22) / std_price_22, 2)
            if std_price_22 != 0
            else 0
        )

        # ================================
        # Diff Z-Score 22 Days
        # ================================

        diff_22 = data - data.shift(22)
        diff_22 = diff_22.dropna()

        mean_diff_22 = diff_22.mean()
        std_diff_22 = diff_22.std()

        current_diff_22 = diff_22.iloc[-1]

        diff_z_score_22 = (
            round((current_diff_22 - mean_diff_22) / std_diff_22, 2)
            if std_diff_22 != 0
            else 0
        )
        ...
        results.append({
            ...
            "Stock Price Z Score 22 Days": stock_price_z_score_22,
...
if not df.empty:
    df_sorted = df.sort_values(
        by="Stock Price Z Score 22 Days",
        ascending=True
    )
