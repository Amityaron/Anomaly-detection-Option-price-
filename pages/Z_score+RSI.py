...
st.set_page_config(
    page_title="RSI + Diff Z-Score 22 Days Buy Signal App",
    layout="wide"
)
...
def compute_diff_zscore_22(series: pd.Series, window: int = 22) -> pd.Series:
    diff_22 = series - series.shift(22)

    diff_mean = diff_22.rolling(window=window).mean()
    diff_std = diff_22.rolling(window=window).std()

    diff_zscore = (diff_22 - diff_mean) / diff_std
...
def add_indicators(
    ...
    df["ZScore"] = compute_diff_zscore_22(df["Close"], window=z_window)
...
def create_zscore_chart(
    ...
    fig.add_trace(go.Scatter(
        ...
        y=df["ZScore"],
        mode="lines",
        name="Diff Z-Score 22 Days"
    ))

    fig.add_hline(
        y=z_threshold,
        line_dash="dash",
        annotation_text=f"Diff Z = {z_threshold}"
    )

    fig.update_layout(
        title=f"{ticker} Diff Z-Score 22 Days",
        xaxis_title="Date",
        yaxis_title="Diff Z-Score 22 Days",
        template="plotly_white",
        height=350,
        hovermode="x unified"
    )
...
st.title("RSI + Diff Z-Score 22 Days Buy Signal Dashboard")
