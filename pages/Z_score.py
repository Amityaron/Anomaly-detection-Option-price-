...
for etf in etfs:
    ...
    # Need more than 22 days so we can calculate diff and its z-score
    ...
    try:
        ...
        if len(data) < 44:
            st.warning(f"Not enough data for {etf} to calculate Diff Z-Score 22 Days. Skipping.")
            continue
        ...
        # Skewness and kurtosis are still calculated from closing prices
...
if not df.empty:
    df_sorted = df.sort_values(by="Diff Z Score 22 Days", ascending=True)
    st.table(df_sorted)
else:
